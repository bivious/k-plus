...
/*
	...
	@description:
		...
		config, help, incoming, log, modules, msgcolors, events, aliases, actions and pics.[br]
		[br]
		...
		this is multi OS friendly and adds a custom icon path to Windows that is not broken
		...
		Overall [b]$escape($file.localdir(pics/panic.png))[/b] is a more correct approach.
	@examples:
		[example]
			KVIrc looks for pictures in $escape($file.localdir(pics))
			panic.png would be translated to $escape($file.localdir(pics/panic.png))
		[/example]
	@seealso:
		[fnc]$escape[/fnc]
*/
